...
public class SecurityConfig {
     ...
     * <li>所有其他未明確指定的請求 ({@code anyRequest()}) 都需要經過認證
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(authz -> authz
                        .requestMatchers("/api/auth/**").permitAll() // Allow public access to /api/auth/ (login,
                                                                     // refresh, logout etc.)
                        .requestMatchers("/public/**").permitAll() // Example: Allow public access to a /public/
                                                                   // directory
                        .requestMatchers("/", "/error", "/favicon.ico", "/swagger-ui/**", "/v3/api-docs/**").permitAll() // Allow
                                                                                                                         // Swagger
                                                                                                                         // and
                                                                                                                         // basic
                                                                                                                         // pages
        ...
        // Add our custom JWT authentication filter before the standard
    ...
    // Note: UserDetailsService bean is already provided by
    // CustomUserDetailsService,
    // and PasswordEncoder is not strictly needed for JWT validation itself if not
    // using form-based login
    // with password checking via AuthenticationManager.
    // If an AuthenticationManager is needed for other purposes, it can be exposed
    // as a Bean.

    // Optional: If you need a PasswordEncoder bean (e.g., for UserDetailsService if
    // you were using it)
    // @Bean
