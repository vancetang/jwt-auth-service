...
public class AuthService {
    ...
    /**
     * 建構 {@code AuthService} 並注入必要的 {@link JwtTokenProvider}。
     *
     * @param jwtTokenProvider 用於 JWT 生成與驗證的服務。
     */
    @Autowired
     ...
     * 並整合 Spring Security 的 {@code AuthenticationManager} 和 {@code UserDetailsService}。
     * </p>
     * <p>
     * 硬編碼的使用者：
     * <ul>
    ...
    public JwtAuthenticationResponse loginUser(LoginRequest loginRequest) {
        ...
        if ("user".equals(username) && "password".equals(password)) {
            ...
            log.info("User '{}' authenticated successfully with role ROLE_USER.", username);
        } else if ("admin".equals(username) && "adminpass".equals(password)) {
            roles = Arrays.asList("ROLE_ADMIN", "ROLE_USER");
            log.info("User '{}' authenticated successfully with roles ROLE_ADMIN, ROLE_USER.", username);
        } else {
        ...
        log.debug("Generated Access Token for {}: {}", username, accessToken);
        log.debug("Generated Refresh Token for {}: {}", username, refreshToken);
        log.info("User {} logged in. AccessToken JTI linked to RefreshToken JTI: {}", username, refreshTokenJti);
     ...
     * <p>
     * 此操作會使傳入的 Access Token 及其關聯的 Refresh Token (如果 `rt_jti` Claim 存在於 Access Token 中)
     * 的 JTI 都被加入黑名單。
     * </p>
     * <p>
    ...
    public void logoutUser(String token) {
        if (token != null && token.startsWith("Bearer ")) {
            ...
            log.info("Attempting to invalidate token for logout.");
            jwtTokenProvider.invalidateToken(actualToken);
            // Note: Refresh token invalidation might also be needed depending on the strategy.
            // For now, only the provided token (assumed Access Token) is invalidated.
        } else {
            log.warn("Logout attempt with invalid or missing Bearer token.");
        }
    }
     ...
     * <ol>
     *   <li>檢查傳入的刷新權杖是否為空。</li>
     *   <li>使用 {@link JwtTokenProvider#validateToken(String)} 驗證刷新權杖的有效性 (簽名、未過期、未被吊銷)。</li>
     *   <li>從刷新權杖中提取 "typ" (類型) 聲明，並驗證其是否為 "Refresh"。</li>
     *   <li>提取使用者 ID (subject)。</li>
     *   <li>根據使用者 ID 獲取使用者角色 (目前為硬編碼，實際應用中應從資料庫查詢)。</li>
     *   <li>使用 {@link JwtTokenProvider#generateAccessToken(String, List)} 生成一個新的存取權杖。</li>
    ...
    public JwtAuthenticationResponse refreshAccessToken(String requestRefreshToken) {
        ...
        if (!jwtTokenProvider.validateToken(requestRefreshToken)) {
            log.warn("Refresh token validation failed. Token: ...{}", requestRefreshToken.length() > 7 ? requestRefreshToken.substring(requestRefreshToken.length() - 7) : requestRefreshToken);
        ...
        if (!"Refresh".equals(tokenType)) {
            log.warn("Invalid token type for refresh. Expected 'Refresh', got '{}'. Token: ...{}", tokenType, requestRefreshToken.length() > 7 ? requestRefreshToken.substring(requestRefreshToken.length() - 7) : requestRefreshToken);
        ...
        if ("user".equals(userId)) {
            ...
            log.warn("Could not determine roles for user ID '{}' from refresh token. Granting no roles for new access token.", userId);
        ...
        if (oldRefreshTokenJti != null && oldRefreshTokenExpiry != null) {
            ...
            log.info("Old Refresh Token with JTI {} has been invalidated during token refresh.", oldRefreshTokenJti);
        } else {
            ...
            log.warn("Old Refresh Token (JTI: {}) was invalidated using full token string due to missing JTI/expiry in claims during refresh.", oldRefreshTokenJti);
        }
        
        log.info("Access token refreshed for user '{}'. New AT linked to new RT JTI {}. Old RT JTI {} invalidated.",
